...
my $pattern = '.*';
my @layouts;
my $layouts_file = $ENV{'HOME'} . '/Library/Application Support/LaunchBar/Actions/Layouts.lbaction/Contents/Scripts/default_layouts.yaml';
...
foreach my $layout (@filtered) {
	$writer->startTag('item', "valid" => "yes", "action" => "layout.sh", "actionArgument" => $layout->{'command'} . $screenOffset, "uid" => $layout->{'name'});
